...
$(function () {
    ...
    $(document).mousemove(function (e) {
        ...
        if (handle) {
            ...
            if (index == 0) {
                ...
                $(".f-hk-4").eq(index).css("left", newLeft);
                //动态改变提示的值
                var myVal = (1 - (parseFloat($(".f-hk-4").eq(index).css("left")) - 8 * index) / ($(".f-w100-4").width() - 8 * $(".detail-slider-box-4").length)) * 100;
                $(".f-hk-4").eq(index).find(".f-range-tips-4").html(parseInt(myVal) + "%");
                //改变信息表值
                var getValNext;
                var getValUp;
                if (index != $(".f-hk-4").length - 1) {
                    getValNext = (1 - (parseFloat($(".f-hk-4").eq(index + 1).css("left")) - 8 * (index + 1)) / ($(".f-w100-4").width() - 8 * $(".detail-slider-box-4").length)) * 100;
                }
                else {
                    getValNext = 0;
                }
                $(".detail-slider-box-4").eq(index).find(".detail-slider-per-4").html(parseInt(myVal - getValNext) + "%");
                getValUp = (1 - (parseFloat($(".f-hk-4").eq(index - 1).css("left")) - 8 * (index - 1)) / ($(".f-w100-4").width() - 8 * $(".detail-slider-box-4").length)) * 100;
                $(".detail-slider-box-4").eq(index - 1).find(".detail-slider-per-4").html(parseInt(getValUp - myVal) + "%");
                //设置其他值
                var money = Math.round(parseInt($("input#fwxze").val()) * parseInt(myVal - getValNext) / 100)
                var num = Math.round(parseInt($("input#fwlsl").val()) * parseInt(myVal - getValNext) / 100)
                var money1 = Math.round(parseInt($("input#fwxze").val()) * parseInt(getValUp - myVal) / 100)
                var num1 = Math.round(parseInt($("input#fwlsl").val()) * parseInt(getValUp - myVal) / 100)
    ...
    for (i = 0; i < $(".f-hk-4").length; i++) {
        ...
        var totalWidth = $(".f-w100-4").width() - 8 * $(".detail-slider-box-4").length;
